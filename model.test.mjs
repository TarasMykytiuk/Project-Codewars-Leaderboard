...
test("mocks a fetchUser function with existing user", async () => {
    ...
    model.apiList.getUser = "https://example.com/test/";
...
test("mocks a fetchUser function with non-existing user", async () => {
    ...
    console.log(model);
    assert(model.errors.includes("Error: User not found - " + model.apiList.getUser + "someone_not_exist"), true)// "'User not found' with url must be in errors log."
